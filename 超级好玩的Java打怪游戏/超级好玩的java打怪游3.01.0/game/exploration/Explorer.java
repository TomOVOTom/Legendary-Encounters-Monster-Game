...
public class Explorer {
    ...
    public static void explore(Player player, Random random, QuestManager questManager) {
        ...
        System.out.println("请选择探索类型：1. 寻找宠物 2. 探索副本");
        int choice = scanner.nextInt();

        if (choice == 1) {
            exploreForPets(player, random);
        } else if (choice == 2) {
            DungeonExplorer.exploreDungeon(player, random, questManager,scanner);
        } else {
            System.out.println("无效的选择！");
        }
    }

    private static void exploreForPets(Player player, Random random) {
