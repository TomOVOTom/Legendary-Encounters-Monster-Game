...
public class BattleResult {
    ...
    public static void handleBattleResult(Player player, Monster monster) {
        ...
        if (monster.attributes.hp <= 0) {
            System.out.println("你战胜了 " + monster.attributes.name + "！ ");
            // 动态调整经验值、金币和技能点的获得量
            int expGain = monster.attributes.level * 10 + random.nextInt(5); // 基础经验值加随机额外经验
            int goldGain = monster.attributes.gold; // 金币奖励
            int skillPointsGain = 5 + monster.attributes.level * 3; // 基础技能点加额外技能点
            ...
            System.out.println("你被 " + monster.attributes.name + " 打败了！");
        }
    }
