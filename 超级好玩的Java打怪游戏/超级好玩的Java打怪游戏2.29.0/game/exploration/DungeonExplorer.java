...
public class DungeonExplorer {
    public static void exploreDungeon(Player player, Random random, QuestManager questManager) {
        ...
        monster.attributes.
                maxHp = (monster.attributes.
                hp * dungeon.getDifficulty());
        monster.attributes.
                hp = monster.attributes.
                maxHp; // Ensure current HP is also updated
        monster.attributes.
                attack += dungeon.getDifficulty() * 2;
        monster.attributes.
                defense += dungeon.getDifficulty() * 2;
        System.out.println("你遇到了怪兽：" + monster.attributes.
                name + " (HP: " + monster.attributes.
                hp + ", 攻击: " + monster.attributes.
                attack + ", 防御: " + monster.attributes.
                defense + ")");
        System.out.println("请选择战斗模式：1. 自动战斗 2. 手动战斗");
        int battleMode = scanner.nextInt();

        if (battleMode == 1) {
            while (monster.attributes.
                    hp > 0 && player.attributes.hp > 0) {
                System.out.println("你的 HP：" + player.attributes.hp + "，" + monster.attributes.
                        name + " 的 HP：" + monster.attributes.
                        hp);
