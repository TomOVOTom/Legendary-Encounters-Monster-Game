        ...
        switch (abilityName) {
            case "毒素":
                ...
                if (monster.stamina >= staminaCost) {
                    damage = 10 + monster.level * 2;
                    player.attributes.hp -= damage;
                    System.out.println(monster.name + " 使用了" + abilityName + " 对玩家造成了" + damage + "点伤害。");
            ...
            case "地震":
                ...
                if (monster.stamina >= staminaCost) {
                    ...
                    player.attributes.hp -= damage;
                    System.out.println(monster.name + " 使用了地震，对玩家造成了 " + damage + " 点伤害。");
            ...
            case "暗影爆发":
                ...
                staminaCost = 10;
                if (monster.stamina >= staminaCost) {
                    damage = 50 + monster.level * 10;
                    System.out.println(monster.name + " 使用了暗影爆发，对玩家造成了 " + damage + " 点伤害。");
                    player.attributes.hp -= damage;
                    player.addStatusEffect(new StatusEffectManager.StatusEffect(StatusEffectType.FEAR, 3 + monster.level / 2, -10));
                    monster.stamina -= staminaCost;
                }
                break;
        }
