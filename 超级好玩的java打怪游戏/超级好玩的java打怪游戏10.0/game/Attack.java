...
public class Attack {
    ...
    public void attack(Monster monster) {
        ...
        if (random.nextInt(150) < monster.dodge) {
            ...
            boolean isCriticalHit = random.nextInt(150) < player.dodge;
            if (isCriticalHit) {
                baseDamage = Math.max((player.attack - monster.defense) * 2, 0);
                System.out.println("你发动了暴击！攻击了 " + monster.name + "，造成了 " + baseDamage + " 点伤害！");
            } else {
                baseDamage = Math.max(player.attack - monster.defense, 0);
                if (baseDamage > 0) {
            ...
            if (player.vampirismLevel > 0 && player.stamina > 0 && totalDamage > 0) {
                int healAmount = (int) (totalDamage * player.vampirismLevel * 0.1);
    ...
    public void attack(Player player) {
        Random random = new Random();
        if (random.nextInt(150) < player.dodge) {
            System.out.println("你躲避了 " + monster.name + " 的攻击！");
        } else {
            if (random.nextInt(150) < monster.dodge) {
                int damage = Math.max((monster.attack - player.defense) * 2, 0);
                player.hp -= damage;
                ...
                player.hp -= damage;
                if (damage > 0) {
                    System.out.println(monster.name + " 攻击了你，造成��� " + damage + " 点伤害！");
                } else {
