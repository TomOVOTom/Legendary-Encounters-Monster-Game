...
public class DamageCalculator {
    ...
    public int applyDamageIncrease(Player attacker, Monster defender, int baseDamage) {
        if (attacker.attributes.damageIncrease > 0 && defender.hp <= defender.maxHp * 0.3) {
            baseDamage *= (1 + attacker.attributes.damageIncrease / 100);
            System.out.println("你的攻击对生命值较低的 " + defender.name + " 造成额外伤害！");
            return baseDamage;
        }
        return baseDamage;
    }
    ...
    public int applyPiercingAttack(Player attacker, Monster defender, int baseDamage) {
        if (random.nextInt(100) < attacker.attributes.piercingAttackRate) {
            System.out.println("你的攻击穿透了 " + defender.name + " 的防御，造成固定伤害！");
            return attacker.attributes.piercingAttackDamage;
        }
        return baseDamage;
    }
